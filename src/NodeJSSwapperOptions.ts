...
export function createNodeJSSwapperOptions(
  ...
  const coinsMap = CoinGeckoSwapPrice.createCoinsMap(wbtc, usdc, usdt);

  if (EVMChains[chain].tokenDecimals != null) {
    if (wbtc != null && EVMChains[chain].tokenDecimals.WBTC != null)
      coinsMap[wbtc].decimals = EVMChains[chain].tokenDecimals.WBTC;
    if (usdc != null && EVMChains[chain].tokenDecimals.USDC != null)
      coinsMap[usdc].decimals = EVMChains[chain].tokenDecimals.USDC;
    if (usdt != null && EVMChains[chain].tokenDecimals.USDT != null)
      coinsMap[usdt].decimals = EVMChains[chain].tokenDecimals.USDT;
  }
  ...
  const returnObj: SwapperOptions = {
    pricing: new CoinGeckoSwapPrice(maxFeeDifference || new BN(5000), coinsMap),
  ...
  returnObj.storage.fromBtcLn = new FileSystemWrapperStorage(
    'storage/fromBtcLn'
  );
